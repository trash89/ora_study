...
--
-- Program description:
-- Turn ON Trace DDL execution for use with TKPROF
...
-- 
--  05/08/08 - SRDJAN
--	  FP None: verify that change has been made
--  04/29/08 - SRDJAN
--	 Initial version
...
prompt
prompt You will be prompted for your own tracing identifier (for files in UDUMP directory, this session only)
accept identname prompt 'Enter your own tracing identifier:'
prompt 
prompt Working, please wait ...
prompt Spooling to file ddl_trace_on_spool.txt

prompt

SET termout OFF
spool ddl_trace_on_spool.txt
...
SELECT upper('&identname') AS identname FROM dual;
...
update "&gg_user"."&setup_table" set value=1 where property='DDL_SQL_TRACING';
commit;
